...
int main(void)
{
  ...
  // TODO: Start timer TIM16					//HAL_TIM_Base starts TIM base generation in interrupt mode
  if( HAL_TIM_Base_Start_IT(&htim16) != HAL_OK) //hal_ok indicates operation successful
  {Error_Handler();}
  ...
  for (uint8_t i=0; i < 6; i++)
  {
	  ...
	  write_to_address(mem_addresses[i], patterns[i]); //writes to memory address starting at 0x0000
  }
  ...
  while (1)
  {
	...
	// TODO: Check button PA0; if pressed, change timer delay
	  if ((GPIOA->IDR & SW0) == 0) //we know PAO = SWO for our purposes here. Check if button pressed
		  if (TimerHalved == 0) //if timer is not already pressed, timer is not halved
		  {
			  //half second timer delay
			  TimerHalved = 1;

			  TIM16->ARR = 500-1; //sets timer period to 500
		  }
		  else
		  {
			  //returns timer back to 1s delay
			  TimerHalved = 0; //else set back to standard
			  TIM16->ARR = 1000-1; //sert timer period to 1000
		  }
